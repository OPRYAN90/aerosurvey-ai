      ...
      try {
        ...
        viewer.loadGUI(() => {
          ...
          window.Potree.loadPointCloud(publicUrl, project.name || 'point cloud', (e: any) => {
            ...
            function processNode(node: any) {
              ...
              for (let i = 0; i < numPoints; i++) {
                try {
                  ...
                  // Apply matrixWorld to get world coordinates
                  const worldPoint = rawPoint.clone().applyMatrix4(node.sceneNode.matrixWorld);
                  allTransformedPoints.push(worldPoint.toArray());

                  // Log first point of each node for debugging
                  if (i === 0) {
                    console.log('🎯 Node point transform:', {
                      nodeName: node.name,
                      raw: rawPoint.toArray(),
                      world: worldPoint.toArray()
                    });
                  ...
                  console.error('❌ Point transform error:', {
                    nodeId: node.name,
                    pointIndex: i,
                    error
                  });
            ...
            // Recursive function to traverse octree
            ...
            function processAllLoadedNodes() {
              if (pointcloud.pcoGeometry?.root) {
                ...
                console.log('✅ Point processing complete:', {
                  totalPoints: allTransformedPoints.length,
                  sample: allTransformedPoints.slice(0, 5)
                });
              }
            }

            // Add event listeners for point cloud loading
            pointcloud.addEventListener('pointcloud_loaded', () => {
              console.log('📡 Point cloud loaded - processing nodes');
              processAllLoadedNodes();
            });

            pointcloud.addEventListener('node_added', (event: any) => {
              console.log('➕ New node added:', event.node?.name);
