class Sidebar{
    ...
    initToolbar(){
        ...
        // Create measurement tools group
        let measurementGroup = $(`<div class="potree-tool-group">
            <div class="potree-tool-group-title">Measurement</div>
            <div class="potree-tool-grid"></div>
        </div>`);
        let measureTools = measurementGroup.find('.potree-tool-grid');

        // Add all existing measurement tools to the grid
        [
            {
                icon: '/icons/angle.png',
                title: '[title]tt.angle_measurement',
                callback: () => {
            ...
            // Add other measurement tools in similar format...
        ].forEach(tool => {
            measureTools.append(this.createToolIcon(
                Potree.resourcePath + tool.icon,
                tool.title,
                tool.callback
            ));
        });

        // Create volume tools group  
        let volumeGroup = $(`<div class="potree-tool-group">
            <div class="potree-tool-group-title">Volume</div>
            <div class="potree-tool-grid"></div>
        </div>`);
        let volumeTools = volumeGroup.find('.potree-tool-grid');

        // Add volume tools
        volumeTools.append(this.createToolIcon(
            Potree.resourcePath + '/icons/volume.svg',
            '[title]tt.volume_measurement',
            () => {
                let volume = this.volumeTool.startInsertion(); 
                this.selectMeasurement(volume);
            }
        ));
        ...
        // Create other tools group
        let otherGroup = $(`<div class="potree-tool-group">
            <div class="potree-tool-group-title">Other</div>
            <div class="potree-tool-grid"></div>
        </div>`);
        let otherTools = otherGroup.find('.potree-tool-grid');

        // Add other tools
        otherTools.append(this.createToolIcon(
            Potree.resourcePath + '/icons/profile.svg',
            '[title]tt.height_profile',
            () => {
                $('#menu_measurements').next().slideDown();
                let profile = this.profileTool.startInsertion();
                this.selectMeasurement(profile);
            }
        ));
        ...
        otherTools.append(this.createToolIcon(
            ...
            () => {
                ...
                this.selectMeasurement(annotation);
            }
        ));
        ...
        elToolbar.append(measurementGroup);
        elToolbar.append(volumeGroup);
        elToolbar.append(otherGroup);

        // Keep existing show/hide measurements code
        { 
    ...
    // Helper method to select measurement in scene tree
